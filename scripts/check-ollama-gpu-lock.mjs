...
// ---------------------------------------------------------------------------
{
  const HOSTS = [
    ...
    // The Koog example's Ollama default is `localhost`, not `127.0.0.1`. Pinned here to record
    // that it is a DIFFERENT file — that path is the reason the Koog harness does not serialise
    // against the Gaunt Sloth one, and pinning it keeps the difference a stated fact rather than
    // something the next reader has to rediscover with a hash.
...
// satisfied by a comment would pass against a harness that does not lock at all.
{
  ...
  for (const rel of ['start-gth-ag-ui.js', `${AG_UI_EXAMPLE_DIR}/start.js`]) {
